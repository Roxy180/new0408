const slides = [
  { text: "柔和贴合，像日常生活里的一层安心。", image: "files/background.png" },
];
...
const questions = [
  ...
  },
  {
    ...
    section: "校园专属",
    title:
      "如果学校里有“熹芮内衣体验站”（可试穿、科普、匿名购买），你会有兴趣去看看吗？",
    type: "single",
...
function renderSlide() {
  ...
  if (slides.length <= 1) {
    ...
    el.dots.innerHTML = slides
      .map(
        (_, i) =>
          `<span class="dot ${i === state.currentSlide ? "active" : ""}"></span>`,
      )
      .join("");
  }
}
...
function nextSlide(step) {
  state.currentSlide =
    (state.currentSlide + step + slides.length) % slides.length;
  renderSlide();
}
      ...
      <div class="scale-row">
        ${[1, 2, 3, 4, 5]
          .map(
            (
              num,
            ) => `<button type="button" class="scale-point ${selected === num ? "active" : ""}" data-value="${num}">
            ${num}
          </button>`,
          )
          .join("")}
...
function renderAllQuestions() {
  const activeQuestions = getActiveQuestions();
  el.questionsContainer.innerHTML = activeQuestions
      ...
      btn.addEventListener("click", () => {
        ...
        if (q.type === "multi") {
          ...
          if (arr.includes(val)) {
            ...
            const base = exclusiveOption
              ? arr.filter((i) => i !== exclusiveOption)
              : arr;
...
function goSubmit() {
  ...
  if (unanswered.length > 0) {
    const targetQuestion =
      unanswered[state.missingJumpIndex % unanswered.length];
...
async function copyWechatId() {
  const value =
    el.copyWechatIdBtn?.getAttribute("data-wechat-id") || "ventusdedian";
  try {
    ...
    el.wechatActionStatus.textContent = ok
      ? `已复制微信号：${value}`
      : "复制失败，请手动复制";
  }
}
...
async function copyBizEmail() {
  const value =
    el.copyBizEmailBtn?.getAttribute("data-biz-email") || "2429026050@qq.com";
  try {
    await navigator.clipboard.writeText(value);
    ...
    const ok = document.execCommand("copy");
    document.body.removeChild(temp);
    el.bizActionStatus.textContent = ok
      ? `已复制邮箱：${value}`
      : "复制失败，请手动复制";
  }
}
...
[el.wechatModal, el.aboutModal, el.policyModal, el.bizModal].forEach(
  (modal) => {
