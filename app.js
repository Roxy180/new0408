...
function renderSlider(q) {
  const current = answerOf(q.id);
  const sliderValue = current ?? 0;
  const valueText = current === undefined ? "请滑动选择" : q.options[current];
  return `
    <div class="slider-wrap">
      <span class="slider-value">${valueText}</span>
      <input type="range" min="0" max="${q.options.length - 1}" step="1" value="${sliderValue}" />
...
function renderAllQuestions() {
  ...
  activeQuestions.forEach((q) => {
    ...
    if (q.type === "slider") {
      ...
      const existing = answerOf(q.id);
      slider.value = existing ?? 0;
      valueTag.textContent = existing === undefined ? "请滑动选择" : q.options[Number(slider.value)];
